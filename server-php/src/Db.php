...
final class Db
{
    ...
    private static function ensureSchema(): void
    {
        ...
        // 기존 테이블에 컬럼 보강 (라이브 마이그레이션) — cursor는 MariaDB 예약어라 백틱 필수
        try { self::$pdo->exec('ALTER TABLE presence ADD COLUMN IF NOT EXISTS `cursor` VARCHAR(48) NULL'); } catch (Throwable $e) {}
        try { self::$pdo->exec('ALTER TABLE presence ADD COLUMN IF NOT EXISTS sel VARCHAR(48) NULL'); } catch (Throwable $e) {}
    }
}
