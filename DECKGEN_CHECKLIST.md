...
### 어드민 신규 8페이지 (Wave C)
- [ ] C1 시스템 상태(health) — 종합 배너+점검토글, 서비스 카드 그리드, 인시던트 테이블
- [ ] C2 데이터 내보내기(exports) — 데이터셋 칩+CSV/JSON, GDPR 추출/파기, 작업 테이블
- [ ] C3 A/B 테스트(abtest) — 실험 카드(변형 전환율 바·우세 배지·승자적용)
- [ ] C4 이메일 로그(emails) — KPI4 + 발송 로그 테이블(재발송)
- [ ] C5 기능 플래그(flags) — 롤아웃% 슬라이더+타겟 셀렉트+ON/OFF
- [ ] C6 약관·정책(policies) — 버전·재동의·게시/초안·이력
- [ ] C7 환불·청구(refunds) — 결제 KPI4 + 결제내역(환불/재청구/인보이스)
- [ ] C8 역할·권한(roles) — 관리자 멤버(역할셀렉트·초대) + 권한 매트릭스
- [ ] C9 내비 개편 — 접이식 사이드바(216↔64) + 6그룹 아코디언 + Material Symbols
