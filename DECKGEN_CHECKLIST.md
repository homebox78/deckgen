...
## 🔄 시안 업데이트 2차 (2026-07-07 16:10, Prototype+Admin diff 1266줄)

### 홈 (Wave A)
- [x] A1 폴더 시스템 — 폴더 칩바(전체/미분류/폴더/휴지통, 개수) + 새 폴더 + 덱 드래그→칩 드롭 이동 + 카드 폴더이동 드롭다운
- [x] A2 즐겨찾기 — 덱 카드 별 토글 + ★즐겨찾기 필터탭 + 상단 정렬
- [x] A3 덱 우클릭 컨텍스트 메뉴 — 열기/즐겨찾기/이름변경/복제/폴더이동/삭제
- [x] A4 휴지통 — 삭제=휴지통 이동, 복원/영구삭제/비우기, 빈 상태, "30일 후 영구삭제"

### 에디터 (Wave B)
- [x] B1 모션 타임라인 — "모션" 토글 + 패널(효과 페이드/떠오름/팝·간격 슬라이더·트랙 리스트·재생) + 요소 등장 애니 바인딩(발표모드 자동재생)
- [x] B2 앱 알림 — 종 버튼(안읽음 배지) + 드롭다운(작성자·본문·슬라이드 점프) + 모두읽음 + 실시간 토스트
- [x] B3 단축키 모달 그룹화 — 탐색/요소편집/발표 3그룹, ? 진입점
- [x] B4 협업 팔로우 — 아바타 클릭 시점 팔로우 + 팔로우중 배지 + 해제
- [x] B5 댓글 @멘션 — @ 입력 자동완성(협업자 후보) + 멘션 알림 토스트
- [x] B6 버전 비교 — 비교 버튼 + diff 패널(제목 추가/삭제/변경 색상·요약)
- [x] (이미 구현) Export 실파일(pptx/figma/png)·업그레이드 모달·플랜 라벨·개요 그리드

### 어드민 신규 8페이지 (Wave C)
- [x] C1 시스템 상태(health) — 종합 배너+점검토글, 서비스 카드 그리드, 인시던트 테이블
- [x] C2 데이터 내보내기(exports) — 데이터셋 칩+CSV/JSON, GDPR 추출/파기, 작업 테이블
- [x] C3 A/B 테스트(abtest) — 실험 카드(변형 전환율 바·우세 배지·승자적용)
- [x] C4 이메일 로그(emails) — KPI4 + 발송 로그 테이블(재발송)
- [x] C5 기능 플래그(flags) — 롤아웃% 슬라이더+타겟 셀렉트+ON/OFF
- [x] C6 약관·정책(policies) — 버전·재동의·게시/초안·이력
- [x] C7 환불·청구(refunds) — 결제 KPI4 + 결제내역(환불/재청구/인보이스)
- [x] C8 역할·권한(roles) — 관리자 멤버(역할셀렉트·초대) + 권한 매트릭스
- [x] C9 내비 개편 — 접이식 사이드바(216↔64) + 6그룹 아코디언 + Material Symbols

---

## 🎨 시안 v2 아이콘 통일 + 스크린샷 반영 (2026-07-07)
- [x] Material Symbols Outlined self-host(2.3MB woff2, src/assets) + @font-face + .mi 클래스 + Icon 컴포넌트
- [x] 앱 전체 이모지·유니코드 글리프 → Material 아이콘 (홈·에디터·어드민·전 패널 ~40파일)
- [x] 어드민 사이드바 그룹 아이콘 22종 Material 이름화 + 레일/그룹/캐럿
- [x] 모노크롬 v2: 설정 브랜드색 스와치 바이올렛/컬러 → 그레이스케일
- [x] 이모지 피커 콘텐츠(EMOJIS)는 유지(삽입 기능), 슬라이드 테마/협업 아바타 색은 콘텐츠라 유지
