...
- [ ] B2 앱 알림 — 종 버튼(안읽음 배지) + 드롭다운(작성자·본문·슬라이드 점프) + 모두읽음 + 실시간 토스트
- [ ] B3 단축키 모달 그룹화 — 탐색/요소편집/발표 3그룹, ? 진입점
- [ ] B4 협업 팔로우 — 아바타 클릭 시점 팔로우 + 팔로우중 배지 + 해제
- [ ] B5 댓글 @멘션 — @ 입력 자동완성(협업자 후보) + 멘션 알림 토스트
- [ ] B6 버전 비교 — 비교 버튼 + diff 패널(제목 추가/삭제/변경 색상·요약)
