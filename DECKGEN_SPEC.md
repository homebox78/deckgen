...
## 14. 관리자 콘솔 (`/admin`) — DeckGenPackage/DeckGen Admin.dc.html 기준

- **인증**: 이메일+비밀번호(config.php `admin_email`/`admin_password`) → **이메일 OTP 6자리 2단계 인증**(기존 §메일 인프라 재사용) → Bearer 관리자 토큰(12시간). 모든 `/api/admin/*`는 토큰 필수.
- **페이지 9종** (다크 사이드바 콘솔, 시안 1:1):
  1. 대시보드 — KPI(오늘 생성/공유 덱/내보내기/평균 생성 시간), 일별 생성 14일 막대, 테마 비율, 파이프라인 p50
  2. 사용자 관리 — 협업 참여자 실데이터(프레즌스·덱 수·최근 접속), 검색, **차단**(collab 입장 403)
  3. 생성 작업 큐 — events 기반 최근 생성 잡(단계·소요·상태)
  4. 오류 로그 — 서버 기록 오류 그룹(횟수 집계), 해결 처리
  5. 감사 로그 — 관리자 액션 append-only(검색/카테고리 필터/CSV)
  6. 공지/배너 — 작성(안내/주의/점검)+미리보기+토글 → **사용자 홈·에디터 상단 배너로 실노출**(`GET /api/banners`)
  7. 템플릿 관리 — 홈 스토리보드 갤러리 항목의 노출/순서/이름/PRO 관리(`GET /api/templates`가 홈에 반영)
  8. 플랜·결제 — 플랜 정의 표시(결제 연동은 2차)
  9. 서비스 설정 — 신규 가입 허용(2차 표시), **일일 생성 한도(IP당)**, **점검 모드**(생성 3종 503+배너), **생성 모델**(§8 모델 오버라이드)
- **이벤트 로깅**: outline/slides/edit/export 호출을 events에 기록(kind·ok·ms·메타) → 대시보드/작업 큐/오류 로그의 실데이터 소스. 실패는 오류 그룹으로 집계.
- 저장소: Node(dev)=`server/data/admin/*.json` · PHP(운영)=MySQL(app_settings/banners/templates/events/audit_log/admin_sessions/blocked_users).
